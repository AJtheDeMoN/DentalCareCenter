...
export default function DefaultGallery() {
    ...
    const handleClick = (index) => {
        // Handle click event here, for example, opening a link
        console.log("Clicked on image with index:", index);
    };

    return (
        <>
        <h1 className="text-3xl font-bold text-center my-5">Our Services</h1>
            <div className="flex justify-center items-center m-2">
                <div className="grid grid-cols-2 gap-4 md:grid-cols-5 lg:max-w-screen-2xl">
                    {images.map((image, index) => (
                        <div key={index} onClick={() => handleClick(index)} className="flex flex-col items-center border border-gray-300">
                            <Link to={imageLinks[index]} className="flex flex-col items-center border border-gray-300">
